...
let acumuladora = 0;
let botonCierraSeccion = document.querySelector(".boton-cerrar-seccion");
let intervalo
...
let contenedorCriticas = document.querySelector(".contenedor-criticas")
let indice;
...
verTextoCompleto.forEach((item) => {item.addEventListener("click", function (){
  ...
  for (i=0;i<textoCriticaCompleta.length;i++){
    contenedorCriticas.style.display = "none";
    botonCierraSeccion.style.display = "none";
...
function cerrarCriticasCompletas(){

  if (contenedorCriticasCompletas.style.display === 'block') {
  contenedorCriticas.style.display = "flex";
 contenedorCriticasCompletas.style.opacity = 0; 
 contenedorCriticasCompletas.style.display = "none";
 textoCriticaCompleta[indice].style.display = "none";     
 botonCierraSeccion.style.display = "block";}
}

botonCierraCriticas.addEventListener("click", cerrarCriticasCompletas);


document.addEventListener("click", function(event) {
  if (event.target.closest(".contenedor-criticas") === null) {
    cerrarCriticasCompletas();
  }
});